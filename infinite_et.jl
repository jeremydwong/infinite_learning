...
# point mass model
c_fr = 0.05 # very sensitive. >.1-> linear. 0.05 -> parabola. 0.01 -> smoothed step.
c_t  = 0.1 # time cost
eyewft = InfiniteModel(Ipopt.Optimizer)

@infinite_parameter(eyewft, t in [0, 1], num_supports=101, derivative_method = OrthogonalCollocation(2))
...
@variable(eyewft, 0.001 <= t_f <= 10, start = 1) # scaling of time by tf.

@objective(eyewft, Min, c_t*integral(fp*v - fn*v + c_fr*(fddotp - fddotm), t)+c_t*t_f) # positive work and force rate 

@constraint(eyewft, deriv(p,t) == t_f*v) # Dynamics
@constraint(eyewft, deriv(v,t) == t_f*(fp + fn))
@constraint(eyewft, t_f*(deriv(fp,t) + deriv(fn,t)) == t_f*fdot) 
@constraint(eyewft, t_f*deriv(fdot,t) == t_f*(fddotp + fddotm))
...
plot!(supports(t), -value(fddotm, ndarray=true))
##
...
@variable(eyesdn, p, Infinite(t), start = (t,w)-> t <= th ? 1*t/th : 1.)
...
@objective(eyewft, Min, integral(fp*v - fn*v + c_fr*fddotp,t)) # positive work and force rate 
...
#objective is work and force-rate
@variable(eyewft, fddotp >= 0, Infinite(t))
@variable(eyewft, fddotm >= 0, Infinite(t))
